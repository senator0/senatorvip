...
function tdcli_update_callback(data)
  ...
  if (data.ID == "UpdateNewMessage") then
    ...
    if msg.content_.ID == "MessageText" then
      if input:match("^[#!/][Pp][Ii][Nn][Gg]$") and is_sudo(msg) or input:match("^[Pp][Ii][Nn][Gg]$") and is_sudo(msg) or input:match("^هستی$") and is_sudo(msg) then
        tdcli.sendText(chat_id, msg.id_, 0, 1, nil, '💈هستم!', 1, 'md')

      end
			
if input:match("^[#!/][Ii][Dd]$") and is_mod(msg) or input:match("^[Ii][Dd]$") and is_mod(msg) or input:match("^ایدی$") then
        tdcli.sendText(chat_id, msg.id_, 0, 1, nil, '> _شناسه سوپر گروه:_ `'..string.sub(chat_id, 5,14)..'`\n> _شناسه شما:_ `'..user_id..'`', 1, 'md')
  end
...
if input:match("^[Ss][Ee][Nn][Aa][Tt][Oo][Rr]$") and is_mod(msg) or input:match("^[Ss]enator$") and is_mod(msg) or input:match("^سناتور$") and is_mod(msg) then
        local text = "🚏 ربات سناتور: \n"
	.."💈 سناتور رباتي قدرتمند جهت مديريت سوپرگروه: \n"
        ...
        .."💈 انچ شدن اتوماتیک بعد از اف شدن\n"
        .."💈  ديباگ شده و قدرتمند \n"
        .."💈  ويرايش و ارتقا: @Lv_t_m \n"
        .."💈  کانال رسمي: @senator_tea \n"
        .."💈  پيام رسان: @Senatorhelpbot \n"
        .." ................................. "
          tdcli.sendText(chat_id, msg.id_, 0, 1, nil, text, 1, 'md')
      end
    ...
    local is_fosh_msg = input:match("کیر") or input:match("کون") or input:match("85") or input:match("جنده") or input:match("ننه") or input:match("ننت") or input:match("مادر جنده") or input:match("قهبه") or input:match("گایی") or input:match("سکس") or input:match("kir") or input:match("kos") or input:match("kon") or input:match("nne") or input:match("nnt") or input:match("حروم") or input:match("لاشی")or input:match("سکسی")or input:match("حشری")or input:match("مفعول")or input:match("فاعل")or input:match("بی پدر")or input:match("گایدم")or input:match("گایدمت")or input:match("خاهر جنده")or input:match("خواهر جنده")or input:match("میگامت")or input:match("کصکش")or input:match("کص")or input:match("کونی")or input:match("کیری")or input:match("کسو")or input:match("حشریم")or input:match("+18")
